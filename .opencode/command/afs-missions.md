...
- Default action is a cheap active listing:
  ...
  `~/src/lab/afs/scripts/afs mission list --path . --status active --limit 10 --json`
...
- Create a mission only for work that outlives this session and has a clear
  ...
  `~/src/lab/afs/scripts/afs mission create --path . --title "..." --summary "..." --owner "<owner>" --next-step "..." --json`

- Before creation, inspect
  `~/src/lab/afs/scripts/afs mission create --help`. If the installed CLI
...
- Update status or next steps when a session materially advances or blocks a
  ...
  `~/src/lab/afs/scripts/afs mission update --path . <mission-id> ... --json`
