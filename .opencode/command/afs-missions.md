...
- Default action is a cheap active listing:
  ...
  `"${AFS_BIN:-${AFS_CLI:-afs}}" mission list --path . --status active --limit 10 --json`

- For one mission's full state:

  `"${AFS_BIN:-${AFS_CLI:-afs}}" mission show --path . <mission-id>`
...
- Create a mission only for work that outlives this session and has a clear
  ...
  `"${AFS_BIN:-${AFS_CLI:-afs}}" mission create --path . --title "..." --summary "..." --owner "<owner>" --next-step "..." --json`

- Before creation, inspect
  `"${AFS_BIN:-${AFS_CLI:-afs}}" mission create --help`. If the installed CLI
...
- Update status or next steps when a session materially advances or blocks a
  ...
  `"${AFS_BIN:-${AFS_CLI:-afs}}" mission update --path . <mission-id> ... --json`
